...
export class MigrateCommand {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        try {
            ...
            const dbInstance = (this.db as any).db as Database.Database;
            ...
            const existingUsers = dbInstance.prepare("SELECT COUNT(*) as count FROM users").get() as any;
            const existingReviews = dbInstance.prepare("SELECT COUNT(*) as count FROM reviews").get() as any;
            ...
            const finalUsers = dbInstance.prepare("SELECT COUNT(*) as count FROM users").get() as any;
            const finalReviews = dbInstance.prepare("SELECT COUNT(*) as count FROM reviews").get() as any;
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    { name: '👥 Utilisateurs', value: `${finalUsers.count} conservés`, inline: true },
                    { name: '📝 Avis', value: `${finalReviews.count} conservés`, inline: true },
                    { name: '🔧 Étapes', value: migrationSteps.join('\n• '), inline: false },
                    { name: '✨ Nouveau support', value: '• Steam\n• Letterboxd\n• **SensCritique** 🎬', inline: false }
                )
