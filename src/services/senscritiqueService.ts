...
export class SensCritiqueService {
     ...
     */
    async getUserReviews(username: string, onlyLatest: boolean = false): Promise<SensCritiqueReview[]> {
        try {
            ...
            const enrichedReviews: SensCritiqueReview[] = [];
     ...
     */
    private async getFullReviewContent(reviewUrl: string): Promise<string | null> {
        try {
     ...
     */
    private parseUserReviews(htmlContent: string, onlyLatest: boolean = false, username: string = ''): SensCritiqueReview[] {
        try {
            const $ = cheerio.load(htmlContent);
            const reviews: SensCritiqueReview[] = [];
            ...
            for (const selector of reviewSelectors) {
                ...
                if (elements.length > 0) {
                    elements.each((index: number, element: any) => {
                        if (onlyLatest && index > 0) return false; // Arrêter après le premier si onlyLatest
                    ...
                    if (reviewsFound) break; // Arrêter si on a trouvé des avis avec ce sélecteur
                }
            }
     ...
     */
    private extractReviewFromElement($element: any, $: any, username: string = ''): SensCritiqueReview | null {
        try {
            // Extraction du titre
            let title = '';
     ...
     */
    private debugPageStructure($: any): void {
        ...
        potentialSelectors.forEach(selector => {
            ...
            if (elements.length > 0) {
                ...
                elements.slice(0, 3).each((i: number, el: any) => {
