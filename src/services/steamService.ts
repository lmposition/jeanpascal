...
import { SteamGame, SteamReview, LetterboxdReview } from '../types/index.js';
...
export class SteamService {
  ...
  async getUserReviews(steamId: string, onlyLatest: boolean = false): Promise<any[]> {
    try {
      ...
      for (let i = 0; i < reviewElements.length; i++) {
        if (onlyLatest && i > 0) break; // Arrêter après le premier avis
  ...
  async isValidSteamId(steamId: string): Promise<boolean> {
  ...
  private extractAppIdFromUrl(url: string): number {
  ...
  private async getGameTitle(appId: number): Promise<string> {
    try {
      if (!appId) return `Jeu Steam ${appId}`;
  ...
  private formatSteamDate(dateString: string): string {
    try {
