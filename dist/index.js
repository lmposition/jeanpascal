        ...
        this.client.on('guildMemberAdd', async (member) => {
            try {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields({ name: 'Utilisateur', value: `<@${member.user.id}>`, inline: true }, { name: 'ID', value: member.user.id, inline: true }, { name: 'Compte créé le', value: `<t:${Math.floor(member.user.createdTimestamp / 1000)}:R>`, inline: false })
                    .setTimestamp()
        ...
        this.client.on('guildMemberRemove', async (member) => {
            try {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setThumbnail(member.user.displayAvatarURL({ size: 128 }))
                    .addFields({ name: 'Utilisateur', value: member.user.tag, inline: true }, { name: 'ID', value: member.user.id, inline: true }, { name: 'A rejoint le', value: member.joinedAt ? `<t:${Math.floor(member.joinedTimestamp / 1000)}:R>` : 'Inconnu', inline: false })
                    .setTimestamp()
